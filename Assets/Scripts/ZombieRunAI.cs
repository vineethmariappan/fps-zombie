...
public class ZombieRunAI : MonoBehaviour
{
    ...
    private void ExecuteMovement(float distanceToPlayer)
    {
        bool shouldRun = DetermineRunState(distanceToPlayer);
        currentSpeed = shouldRun ? runSpeed : walkSpeed;

        Vector3 direction = CalculateDirectionToPlayer();
        MoveTowardsPlayer(direction);
        RotateTowardsPlayer(direction);
        UpdateMovementAnimations(shouldRun);
    }

    private bool DetermineRunState(float distance)
    {
    ...
    private void UpdateMovementAnimations(bool isRunning)
    {
        if (animator != null)
        {
